...
struct AuthManager{
    ...
    static func registerUserWithFirebase(email: String, password: String, fullname: String, hasSeenOnboardingPage: Bool, completion: @escaping DatabaseCompletion){
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            if let error = error {
                print("DEBUG: Error occured in registerUserWithFirebase() - \(error.localizedDescription)")
            ...
            Database.database().reference().child("users").child(uid).updateChildValues(values) { (error, ref) in
                print("Successfully regitered and created user in registerUserWithFirebase() ")
            }
        }
    }
}
