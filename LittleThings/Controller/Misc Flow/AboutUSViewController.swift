...
class AboutUSViewController: UIViewController {
    ...
    @IBAction func exitButtonTapped(_ sender: Any) {dismiss(animated: true)}
    @IBAction func twitterButtonTapped(_ sender: Any) {twitterTapped()}
    @IBAction func gitHubButtonTapped(_ sender: Any) {githubTapped()}
    @IBAction func linkedinButtonTapped(_ sender: Any) {linkedInTapped()}
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        if !stacks.isEmpty{
            stacks.forEach{$0.setBorderColorForObject(using: Constants.whiteOffblack!)}
        }
    }
